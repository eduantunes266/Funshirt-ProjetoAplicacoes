...
class CustomerImageController extends Controller
{
    ...
    {
        $images = TshirtImage::where('customer_id', $request->user()->customer->id)
            ->where('custom', 1)
            ->latest()
            ->get();
    ...
    {
        abort_unless(
            $tshirtImage->customer_id === $request->user()->customer->id,
            403
        );
...
{
    abort_unless($tshirtImage->customer_id === $request->user()->customer->id, 403);
    ...
    public function update(Request $request, TshirtImage $tshirtImage)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $tshirtImage->image_url = $request->file('image')->store('tshirt_images_private/custom', 'local');
        }
